var quest = {
    ...
    makeBasicChest : function(){
        return land.createMob("CHS", 80, 80, "ничего", "Сундук !! Как он здесь оказался?", [drops.createDrop("candies", 300 + random.getRandomIntUpTo(500)), drops.createDrop("object", "key", true), drops.createDrop("object", "boots", random.oneChanceOutOf(3)), drops.createDrop("object", "swampMap", random.oneChanceOutOf(3)), drops.createDrop("object", "hutMap", random.oneChanceOutOf(3))]);
    },
    
    makeOpenChest : function(){
        return land.createMob("CHS", 1, 1, "ничего", "Открытый сундук, полный конфет. Было ничьё стало моё !", [drops.createDrop("candies", 6000 + random.getRandomIntUpTo(2000))]);
    },
    
    makeImp : function(){
        var hp = 15 + random.getRandomIntUpTo(10);
        return land.createAlly("IMP", hp, hp, "всё его тело", "Чертёнок.", []);
    },
    
    makeOrc : function(){
        return land.createAlly("ORC", 30, 30, "Дубина", "Орк. Выглядит тупо, как и его оружие.", []);
    },
    
    makeDraugr : function(){
        return land.createAlly("DRG", 35, 35, "разные кости", "Драугр. Он кажется мёртвым, но ещё двигается..", []);
    },
    
    makeChupacabra : function(){
        return land.createAlly("CBA", 30, 30, "Клыки", "Чупакабра. Что она забыла в фэнтезийной игре? !", []);
    },
    
    makeGolem : function(){
        return land.createAlly("GOL", 100, 100, "валуны", "Голем. Твёрдый, твёрдый голем.", []);
    },
    
    makeChimera : function(){
        var hp = 50 + random.getRandomIntUpTo(7);
        return land.createAlly("CHI", hp, hp, "огонь", "Химера: Лев, змея и козел (в обоих смыслах ;) ) одновременно.", []);
    },
    
    makeCandyMonster : function(){
        var hp = 80 + 5*sword.specialPower;
        
        return land.createAlly("CND", hp, hp, "взрывные конфеты", "Конфетный монстр. Он бросает конфеты в своих врагов.", []);
    },
