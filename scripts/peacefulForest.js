var peacefulForest = {
    ...
    // Functions
    onload : function(){
        land.addLand("Мирный лес", 30, 0, this.load.bind(this), this.getText.bind(this), this.move.bind(this));
    },
    ...
    load : function(){
        ...
        for(var i = 1; i < quest.things.length; i++){
            if(random.flipACoin()){
                ...
                else quest.things[i] = land.createMob("|||", 5, 5, "ничего", "Дерево. Оно иногда дропает конфету.", [drops.createDrop("candies", random.getRandomIntUpTo(1)), drops.createDrop("object", "key", random.oneChanceOutOf(2))]);
            }
        }
    ...
    makeWoodPoney : function(){
        return land.createMob("WPY", 12, 12, "копыта", " лесной пони ! Это же пони ! Это же лес !", [drops.createDrop("candies", 42)]);
    }
