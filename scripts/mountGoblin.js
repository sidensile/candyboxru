var mountGoblin = {
    ...
    // Functions
    onload : function(){
        land.addLand("Гоблиногорье", 28, 1, this.load.bind(this), this.getText.bind(this));
    },
        ...
        for(var i = 1; i < quest.things.length; i++){
            if(random.flipACoin()){
                ...
                if(i < 12 || i > 15){
                    ...
                    else if(random.oneChanceOutOf(7)) quest.things[i] = land.createMob("GSB", 20, 5 + random.getRandomIntUpTo(5), "когти", "Больной гоблин, он плохо пахнет.", [drops.createDrop("candies", 3 + random.getRandomIntUpTo(3))]);
                    // Else we spawn a GOB
                    else quest.things[i] = land.createMob("GOB", 20, 20, "когти", "Противный гоблин.", [drops.createDrop("candies", 3 + random.getRandomIntUpTo(3))]);
                }
                else{
                    quest.things[i] = land.createMob("GTB", 30, 30, "dagger", "Стойкий гоблин. О, у него есть кинжал.", [drops.createDrop("candies", 9 + random.getRandomIntUpTo(9)), drops.createDrop("object", "key", random.oneChanceOutOf(2)), drops.createDrop("object", "boots", random.oneChanceOutOf(5)), drops.createDrop("object", "swampMap", random.oneChanceOutOf(5)), drops.createDrop("object", "hutMap", random.oneChanceOutOf(5))]);
                }
            }
        }
    },
